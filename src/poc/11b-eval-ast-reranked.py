#!/usr/bin/env python3
"""
POC 11b: Eval — Vectorize+D1 WITH reranker vs AI Search native

Same as 10b but adds bge-reranker-base cross-encoder after RRF merge.
Tests whether reranking closes the MRR gap (0.703 → target 0.65+).
"""
...
D1_ID = "71b99e5f-817e-42f6-b96b-540e2cd9612f"
AISEARCH_INSTANCE = "lumae-eval-bare"

QUERIES_PATH = Path(__file__).resolve().parents[2] / "benchmarks" / "lumae_golden_queries.json"
RESULTS_PATH = Path(__file__).resolve().parents[2] / "benchmarks" / "lumae_eval_ast_reranked.json"
...
def run():
    print("POC 11b: Eval — AST+Reranker vs AI Search\n")
    ...
    variants = {"ast_reranked": [], "aisearch": []}
    latencies = {"ast_reranked": [], "aisearch": []}
    per_query = []
    ...
    for i, q in enumerate(queries):
        ...
        try:
            ...
            latencies["ast_reranked"].append(lat)
            sc = score_query(vr, q.get("relevant", []), LIMIT)
            variants["ast_reranked"].append(sc)
            pq["ast_reranked"] = sc
        except Exception as e:
            print(f"    Vec error {i}: {e}")
            variants["ast_reranked"].append({"recall_at_5": 0, "recall_at_10": 0, "mrr": 0, "ndcg_at_10": 0})
            latencies["ast_reranked"].append(0)
            pq["ast_reranked"] = variants["ast_reranked"][-1]
    ...
    prev_line_rerank = {"recall_at_5": 0.833, "recall_at_10": 0.871, "mrr": 0.703, "ndcg_at_10": 0.717}

    print(f"\n{'='*80}")
    print(f"  AST+Reranker vs AI Search vs Line+RR (line-based reranked, previous)")
    print(f"  {len(queries)} queries, top-{LIMIT}")
    print(f"{'='*80}\n")

    print(f"  {'Metric':<16} {'Line+RR':>10} {'+ Rerank':>10} {'AI Search':>10} {'AST Δ':>10}")
    print(f"  {'-'*56}")

    for m in metrics:
        prev = prev_line_rerank[m]
        reranked = aggs["ast_reranked"][m]
        ais = aggs["aisearch"][m]
        delta = reranked - prev
        print(f"  {m:<16} {color(prev):>19} {color(reranked):>19} {color(ais):>19} {delta:>+10.3f}")

    vl = aggs["ast_reranked"]["p95_latency_ms"]
    ...
    for t in sorted(types):
        vr = sum(types[t]["ast_reranked"]) / len(types[t]["ast_reranked"])
        ar = sum(types[t]["aisearch"]) / len(types[t]["aisearch"])
        print(f"  {t:<20} {vr:>10.3f} {ar:>10.3f} {len(types[t]['ast_reranked']):>6}")

    # Verdict
    print(f"\n{'='*80}")
    vw, aw = 0, 0
    for m in metrics:
        if aggs["ast_reranked"][m] > aggs["aisearch"][m] + 0.01: vw += 1
        elif aggs["aisearch"][m] > aggs["ast_reranked"][m] + 0.01: aw += 1
    if vw > aw:
        print(f"  VERDICT: AST+Reranker wins {vw}/{len(metrics)} metrics")
    elif aw > vw:
    ...
    output = {"query_count": len(queries), "limit": LIMIT, "variants": aggs, "per_query": per_query,
              "previous_no_rerank": prev_line_rerank}

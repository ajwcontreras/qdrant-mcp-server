// POC 28C: Per-chunk queue consumer = HyDE generate + code embed + 12 question embed + upsert + D1.
// Throwaway worker — proves the per-chunk pipeline before integrating into the canonical worker.
...
async function processChunk(env: Env, msg: QMsg) {
  ...
  // HyDE question embeddings (one batched call)
  const hydeVecs = await vertexEmbed(env, questions);

  // Build vectors + D1 rows
  ...
  // D1 inserts (sequential — D1 batch is best but for 13 rows direct is fine)
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health") return Response.json({ ok: true, service: "28c-consumer" });

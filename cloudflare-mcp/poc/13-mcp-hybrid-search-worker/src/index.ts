...
async function seed(env: Env) {
  ...
  const chunks = [
    ...
    },
    {
      ...
      start_line: 1,
      end_line: 20,
      symbols: "update_market_rates fred_rates mortgage_rates",
  ...
  await env.DB.batch(chunks.map((chunk) => env.DB.prepare(`
    INSERT OR REPLACE INTO chunks (
      vector_id, chunk_identity, file_path, start_line, end_line, symbols, snippet, match_reason
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    chunk.vector_id,
  ...
  await env.DB.batch(chunks.map((chunk) => env.DB.prepare(`
    INSERT INTO chunks_fts (vector_id, file_path, symbols, snippet) VALUES (?, ?, ?, ?)
  `).bind(chunk.vector_id, chunk.file_path, chunk.symbols, chunk.snippet)));
}
...
async function hydratedSearch(env: Env, query: string) {
  ...
  for (const match of vectorResults.matches || []) {
    ...
    if (!chunk) continue;
    byChunk.set(chunk.chunk_identity, {
      file_path: chunk.file_path,
      start_line: chunk.start_line,
      end_line: chunk.end_line,
  ...
  if (ftsQuery) {
    const fts = await env.DB.prepare(`
      SELECT c.vector_id, c.chunk_identity, c.file_path, c.start_line, c.end_line, c.snippet, c.match_reason
      FROM chunks_fts f
      JOIN chunks c ON c.vector_id = f.vector_id
      WHERE chunks_fts MATCH ?
      LIMIT 5
    `).bind(ftsQuery).run<ChunkRow>();
    for (const chunk of fts.results || []) {
      const existing = byChunk.get(chunk.chunk_identity);
      if (existing) {
        existing.score += 1;
        existing.match_reasons.push(`fts: ${ftsQuery}`);
      } else {
        byChunk.set(chunk.chunk_identity, {
          file_path: chunk.file_path,
          start_line: chunk.start_line,
...
function createServer(env: Env) {
  const server = new McpServer({
    name: "cfcode-poc-13-mcp-hybrid-search",
    version: "0.1.0",
  });
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/health") {
      return json({ ok: true, service: "cfcode-poc-13-mcp-hybrid-search" });
    }

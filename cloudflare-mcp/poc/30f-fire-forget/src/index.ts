// POC 30F: fire-and-forget producer via JobOrchestratorDO + storage alarm.
//
// First attempt used ctx.waitUntil to drive the dual fan-out after the
// producer returned. That hit the ~30s waitUntil cap — code went live but
// HyDE (70s) never completed because the orchestrator was killed before
// its .then() ran.
//
// This version: producer enqueues the job into a JobOrchestratorDO (one
// instance per job_id), the DO records the input in its own storage and
// schedules an alarm for "now". The alarm handler runs inside the DO's
// request lifetime (much larger than ctx.waitUntil's cap) and drives both
// fan-outs to completion. Producer returns in <100ms; clients poll
// /jobs/:id/status as before.
...
type Env = {
  ...
  JOB_ORCHESTRATOR_DO: DONamespaceLike;
...
// ── Pure dual fan-out logic (called by the orchestrator DO's alarm) ──
async function runDualFanout(env: Env, input: IngestReq, records: SourceRecord[], tStart: number): Promise<void> {
  ...
  // Drive both fan-outs to completion inside the orchestrator DO's alarm
  // request lifetime — not bounded by ctx.waitUntil's ~30s post-response cap.
  const [codeSettled, hydeSettled] = await Promise.allSettled([codeFanout, hydeFanout]);
  const codeRes = codeSettled.status === "fulfilled" ? codeSettled.value : { results: [] as CodeShardResult[], completed: 0, errors: 1, wall_ms: Date.now() - tStart };
  const hydeRes = hydeSettled.status === "fulfilled" ? hydeSettled.value : { results: [] as HydeShardResult[], hyde_completed: 0, errors: 1, wall_ms: Date.now() - tStart };
...
// ── JobOrchestratorDO: receives the input, schedules an alarm, drives the fan-out ──
type StoredJob = { input: IngestReq; records: SourceRecord[]; t_start: number };
export class JobOrchestratorDO extends DurableObject<Env> {
  async fetch(req: Request): Promise<Response> {
    const u = new URL(req.url);
    if (u.pathname === "/start" && req.method === "POST") {
      const input = await req.json() as IngestReq;
      const records = parseRecords(input.artifact_text);
      const stored: StoredJob = { input, records, t_start: Date.now() };
      await this.ctx.storage.put("job", stored);
      // Schedule alarm for "now" — DO's alarm handler runs in its own request lifetime.
      await this.ctx.storage.setAlarm(Date.now() + 10);
      return json({ ok: true, queued: true, records: records.length });
    }
    return json({ error: "not_found" }, 404);
  }
  async alarm(): Promise<void> {
    ...
    if (!stored) return;
    try {
      await runDualFanout(this.env, stored.input, stored.records, stored.t_start);
    } catch (e) {
      console.error("orchestrator alarm failed:", e instanceof Error ? e.message : e);
      await this.env.DB.prepare(`UPDATE jobs SET status = 'failed', failed = failed + 1 WHERE job_id = ?`)
        .bind(stored.input.job_id).run();
    } finally {
      // Free up the DO's storage now that the job is durable in D1.
      await this.ctx.storage.delete("job");
    }
  }
}

// ── Producer endpoint: enqueue + return immediately ──
async function ingestSharded(env: Env, input: IngestReq): Promise<Response> {
  ...
  // Synchronous setup: artifact to R2, initial job row to D1.
  ...
  ).bind(input.job_id, input.repo_slug, input.indexed_path, input.active_commit, input.artifact_key, records.length, includeHyde ? "running" : "skipped", startedAt).run();

  // Hand off to the orchestrator DO. DO schedules its alarm and returns fast.
  const stub = env.JOB_ORCHESTRATOR_DO.get(env.JOB_ORCHESTRATOR_DO.idFromName(`cfcode:job:${input.job_id}`));
  const r = await stub.fetch("https://orchestrator.internal/start", {
    method: "POST", headers: { "content-type": "application/json" },
    body: JSON.stringify(input),
  });
  if (!r.ok) {
    const t = await r.text();
    return json({ ok: false, error: `orchestrator enqueue failed: ${r.status} ${t.slice(0, 200)}` }, 500);
  }
  ...
  return json({
    ...
    status: "running",
    code_status: "running",
    hyde_status: includeHyde ? "running" : "skipped",
    started_at: startedAt,
    poll_url: `/jobs/${input.job_id}/status`,
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    const u = new URL(request.url);
    if (u.pathname === "/health") return json({ ok: true, service: "cfcode-poc-30f-fire-forget" });

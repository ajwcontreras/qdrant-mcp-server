 ...
 *
 * Proves:
 *   An unauthenticated remote MCP Worker can expose a search tool that queries
 *   Vectorize and hydrates snippets from D1.
 */
...
const pocDir = path.resolve(__dirname, "../poc/12-mcp-search-vectorize-worker");
const cfKeysPath = path.join(repoRoot, ".cfapikeys");
const workerName = "cfcode-poc-12-mcp-search-vectorize";
const indexName = "cfcode-poc-12-mcp-search-vectorize";
const dbName = "cfcode-poc-12-mcp-search-vectorize";
...
async function callSearch(mcpUrl) {
  ...
  const client = new Client({ name: "cfcode-poc-12-client", version: "0.1.0" });
  ...
  try {
    ...
    const response = await client.callTool({
      name: "search",
      arguments: { query: "borrower upload document handler" },
    });
...
async function searchUntilVisible(mcpUrl) {
  ...
  throw new Error(`MCP search did not return hydrated app.py result: ${JSON.stringify(lastPayload)?.slice(0, 800)}`);
}

async function main() {
  console.log("POC 12: MCP Search Over Vectorize\n");
  ...
  try {
    ...
    const { toolNames, payload } = await searchUntilVisible(mcpUrl);

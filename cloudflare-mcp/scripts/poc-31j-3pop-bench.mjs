#!/usr/bin/env node
// POC 31J: Three-population fan-out. Code (Vertex) + DS gen (DeepSeek) + Hyde embed (Vertex)
...
const T = "31j", wn = `cfcode-poc-${T}-3pop`, dn = `cfcode-poc-${T}-3pop`, r2b = `cfcode-poc-${T}-3pop`, vi = `cfcode-poc-${T}-3pop`;
...
async function main() {
  ...
  console.log("\n--- POST /ingest-sharded (4c+64q+16h max parallel) ---"); const t0 = Date.now();
  const ing = await (await fetch(u + "/ingest-sharded", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ job_id: jid, repo_slug: repoS, indexed_path: luma, active_commit: T, artifact_key: ak, artifact_text: at, code_shard_count: 4, qgen_shard_count: 64, hemb_shard_count: 16, code_batch_size: 500, hemb_batch_size: 500, hyde: true }) })).json();
  ...
  console.log((pass ? "PASS" : "FAIL") + " POC 31J (code=" + (job?.completed||0) + " qgen=" + (job?.questions_generated||0) + " hyde=" + (job?.hyde_completed||0) + ")");
  try { run("npx", ["wrangler", "delete", "--name", wn, "--force"], { q: 1, ok: 1 }); run("npx", ["wrangler", "vectorize", "delete", vi, "--force"], { q: 1, ok: 1 }); run("npx", ["wrangler", "r2", "bucket", "delete", r2b], { q: 1, ok: 1 }); run("npx", ["wrangler", "d1", "delete", dn, "--skip-confirmation"], { q: 1, ok: 1 }); } catch {}

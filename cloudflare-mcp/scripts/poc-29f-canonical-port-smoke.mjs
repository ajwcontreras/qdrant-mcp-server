#!/usr/bin/env node
/**
 * POC 29F: canonical worker port smoke test
 *
 * Deploys workers/codebase (canonical) with the new IndexingShardDO + DO
 * binding to a THROWAWAY worker name. Runs lumae through /ingest-sharded.
 * Verifies /search still works on the indexed data. Cleans up.
 *
 * Does NOT touch production cfcode-codebase-lumae-fresh.
 *
 * Pass criteria:
 *   - Canonical worker deploys cleanly with DO binding
 *   - /ingest-sharded returns ok=true with all chunks completed
 *   - /search returns matches (proves Vectorize+D1 roundtrip)
 *   - /health and /metrics still work (legacy endpoints intact)
 *
...
const outDir = path.resolve(__dirname, "../poc/29f-canonical-port");
...
const TAG = "29f";
const workerName = `cfcode-poc-${TAG}-canonport`;
const dbName = `cfcode-poc-${TAG}-canonport`;
...
function writeWranglerConfig(d1Id) {
  // Build a complete config with DO + R2 + D1 + Vectorize + queue (canonical worker
  // still has queue consumer in code, even if /ingest-sharded doesn't use it)
  const cfg = {
...
async function main() {
  console.log("POC 29F: canonical worker port smoke\n");
  const checks = {
  ...
  const bench = {
    poc: "29f",
    note: "canonical worker (workers/codebase) port smoke — DO binding + /ingest-sharded + legacy /search",
  ...
  try {
    ...
    // Verify legacy endpoints still respond
    const h = await fetchJson(`${baseUrl}/health`);
    checks.healthOK = h.status === 200 && h.body?.ok === true;
    console.log(`/health: ${JSON.stringify(h.body)}`);
    const m = await fetchJson(`${baseUrl}/metrics`);
    checks.metricsOK = m.status === 200 && m.body?.ok === true;
    console.log(`/metrics: ${JSON.stringify(m.body)}`);
    ...
    const ing = await fetchJson(`${baseUrl}/ingest-sharded`, {
      ...
      body: JSON.stringify({
        job_id: jobId, repo_slug: repoSlug, indexed_path: lumaePath, active_commit: "smoke",
    ...
    if (ing.status !== 200) console.log(`body: ${JSON.stringify(ing.body).slice(0, 400)}`);
    ...
    console.log(`completed=${bench.completed}/${chunks.length} cps=${bench.chunks_per_sec} speedup=${bench.speedup_vs_baseline}x`);

    console.log("\n--- POST /search (Vectorize is eventually consistent, retry up to 60s) ---");
    let sq;
    ...
    while (Date.now() < searchDeadline) {
      sq = await fetchJson(`${baseUrl}/search`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ repo_slug: repoSlug, query: "hello world", topK: 3 }),
      });
      const matches = sq.body?.matches?.length ?? 0;
      const vecReturned = sq.body?.vectorize_returned ?? 0;
      console.log(`/search status=${sq.status} vectorize_returned=${vecReturned} d1_matches=${matches}`);
      if (matches > 0) break;
    ...
    fs.writeFileSync(path.join(outDir, "bench-29f.json"), JSON.stringify(bench, null, 2), "utf8");
    checks.benchWritten = true;
    console.log(`\nbench written: ${path.join(outDir, "bench-29f.json")}`);
  } finally {
    cleanup(configPath);
  ...
  console.log(`\nbench-29f.json: ${JSON.stringify(bench, null, 2)}`);
  const allPass = Object.values(checks).every(Boolean);
  console.log(`\n${allPass ? "PASS POC 29F" : "FAIL POC 29F"}`);

 ...
 *
 * Proves:
 *   D1 can store chunk metadata and query it by chunk identity and repo.
 */
...
const pocDir = path.resolve(__dirname, "../poc/03-d1-metadata-smoke");
const cfKeysPath = path.join(repoRoot, ".cfapikeys");
const workerName = "cfcode-poc-03-d1-metadata";
const dbName = "cfcode-poc-03-d1-metadata";
...
async function main() {
  console.log("POC 03: D1 Metadata Smoke\n");
  ...
  try {
    ...
    checks.deploy = true;
    console.log(`Deployed metadata Worker: ${baseUrl}`);
    await waitForHealth(baseUrl);

    const init = await fetchJson(`${baseUrl}/init`, { method: "POST" });
    checks.initSchema = init.ok === true;

    const record = {
      chunk_identity: "repo:lumae-fresh:path:app.py:symbol:upload:L1-L8:v1",
    ...
    const inserted = await fetchJson(`${baseUrl}/chunk`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(record),
    });
    checks.insert = inserted.ok === true && inserted.chunk_identity === record.chunk_identity;

    const got = await fetchJson(`${baseUrl}/chunk?id=${encodeURIComponent(record.chunk_identity)}`);
    checks.getById = got.ok === true && got.row?.file_path === "app.py" && got.row?.start_line === 1;

    const listed = await fetchJson(`${baseUrl}/chunks?repo=lumae-fresh`);
    checks.listByRepo = listed.ok === true && listed.rows?.some((row) => row.chunk_identity === record.chunk_identity);
  } finally {
    try {

#!/usr/bin/env node
/**
 * POC 29D: sharded DO fan-out + round-robin SA bench
 *
 * Provisions throwaway resources, deploys 29d-shard-fanout worker, sets BOTH
 * GEMINI_SERVICE_ACCOUNT_B64 + GEMINI_SERVICE_ACCOUNT_B64_2 secrets, runs lumae
 * through /ingest-sharded, captures per-shard metrics. Compares to 29A baseline.
 *
 * Pass criteria:
 *   - All chunks processed, 0 errors
 *   - SHARD_COUNT shards return metrics
 *   - Per-SA call count balanced (delta ≤ 1)
 *   - chunks_per_sec ≥ 3× 29A baseline (≥18 cps)
 *
...
const TAG = "29d";
const workerName = `cfcode-poc-${TAG}-shardfan`;
const dbName = `cfcode-poc-${TAG}-shardfan`;
...
async function main() {
  console.log("POC 29D: sharded DO fan-out bench\n");
  const checks = {
  ...
  const bench = {
    poc: "29d",
    note: `sharded DO fan-out, SHARD_COUNT=${SHARD_COUNT}, BATCH_SIZE=${BATCH_SIZE}, NUM_SAS=2`,
    baseline_chunks_per_sec: BASELINE_CPS,
    target_chunks_per_sec: BASELINE_CPS * 3,
  ...
  try {
    ...
    console.log(`body size: ${(ingestBody.length / 1024).toFixed(1)}KB`);
    const t0 = Date.now();
    ...
    console.log(`response status=${ing.status} clientWallMs=${clientWallMs}`);
    if (ing.status !== 200 || !ing.body?.ok) {
      console.log(`response body: ${JSON.stringify(ing.body).slice(0, 500)}`);
    }
    checks.ingestSucceeded = ing.status === 200 && !!ing.body?.ok;
    ...
    // Per-SA call distribution
    for (const s of bench.shards) {
      bench.sa_call_counts[s.sa_index] = (bench.sa_call_counts[s.sa_index] || 0) + s.vertex_calls;
    }
    checks.allShardsReported = bench.shards.length === SHARD_COUNT;
    const sa0 = bench.sa_call_counts[0] || 0;
    const sa1 = bench.sa_call_counts[1] || 0;
    checks.saBalanced = Math.abs(sa0 - sa1) <= 1;
    checks.speedup3x = bench.chunks_per_sec >= BASELINE_CPS * 3;

    console.log(`\nresult: completed=${bench.completed}/${bench.chunks} failed=${bench.failed}`);
    console.log(`vertex_calls=${bench.vertex_calls_total} wall_ms=${bench.wall_ms} chunks_per_sec=${bench.chunks_per_sec} speedup=${bench.speedup_vs_baseline}x`);
    console.log(`sa_calls: sa0=${sa0} sa1=${sa1}`);
    console.log("per-shard:");
    for (const s of bench.shards) {
      console.log(`  shard ${s.shard_index} sa=${s.sa_index}: chunks=${s.chunks_done} verts=${s.vertex_calls} v=${s.vertex_ms}ms vec=${s.vectorize_ms}ms d1=${s.d1_ms}ms err=${s.errors}`);
    }

    bench.finished_at = new Date().toISOString();
    fs.writeFileSync(path.join(pocDir, "bench-29d.json"), JSON.stringify(bench, null, 2), "utf8");
    checks.benchWritten = true;
    console.log(`\nbench written: ${path.join(pocDir, "bench-29d.json")}`);
  } finally {
    cleanup(configPath);
  ...
  console.log(`\n${allPass ? "PASS POC 29D" : "FAIL POC 29D"}`);

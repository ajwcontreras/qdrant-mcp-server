#!/usr/bin/env node
/**
 * POC 30A: HyDE + code in parallel inside IndexingShardDO
 *
 * Same harness as 29D but worker is the 30a-hyde-shard variant which runs
 * DeepSeek HyDE generation in parallel with Vertex code embedding inside each
 * shard. Per-shard: Promise.all([code_path, hyde_path]). Per-chunk hyde:
 * Promise.all over chunks for DeepSeek calls (no rate limits).
 *
 * Pass criteria:
 *   - All chunks have a code vector AND ~12 hyde vectors each (≥99% completion)
 *   - 0 errors
 *   - chunks_per_sec ≥ 30 (5× 29A baseline) — HyDE adds work but parallel hides it
 *   - vectors_per_sec ≥ 400 (true throughput including HyDE)
 *
...
const pocDir = path.resolve(__dirname, "../poc/30a-hyde-shard");
...
const TAG = "30a";
const workerName = `cfcode-poc-${TAG}-hydeshard`;
const dbName = `cfcode-poc-${TAG}-hydeshard`;
...
async function main() {
  console.log("POC 30A: HyDE + code parallel in shards\n");
  const checks = {
    ...
    chunksBuilt: false,
    ingestOK: false,
    codeCompleteHigh: false,
    hydeCompleteHigh: false,
    speedup5x: false,
    benchWritten: false,
    cleanedUp: false,
  };
  const bench = {
    poc: "30a",
    note: "HyDE + code parallel in shards (per-chunk Promise.all, per-shard fanout DeepSeek)",
  ...
  try {
    ...
    console.log(`artifact: ${(artifactText.length / 1024).toFixed(1)}KB`);

    console.log("\n--- POST /ingest-sharded-hyde ---");
    const t0 = Date.now();
    const ing = await fetchJson(`${baseUrl}/ingest-sharded-hyde`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        job_id: jobId, repo_slug: repoSlug, indexed_path: lumaePath, active_commit: "30a",
        artifact_key: artifactKey, artifact_text: artifactText,
        shard_count: 16, batch_size: 100, hyde: true,
      }),
    }, 1);
    const clientWallMs = Date.now() - t0;
    console.log(`status=${ing.status} clientWall=${clientWallMs}ms`);
    if (ing.status !== 200) console.log(`body: ${JSON.stringify(ing.body).slice(0, 600)}`);
    checks.ingestOK = ing.status === 200 && ing.body?.ok === true;
    bench.completed = ing.body?.completed ?? 0;
    bench.hyde_completed = ing.body?.hyde_completed ?? 0;
    bench.failed = ing.body?.failed ?? 0;
    bench.vertex_calls_total = ing.body?.vertex_calls_total ?? 0;
    bench.deepseek_calls_total = ing.body?.deepseek_calls_total ?? 0;
    ...
    bench.vectors_per_sec = ing.body?.vectors_per_sec ?? +((bench.completed + bench.hyde_completed) / (bench.wall_ms / 1000)).toFixed(3);
    bench.speedup_vs_baseline = +(bench.chunks_per_sec / BASELINE_CPS).toFixed(3);
    bench.shards = ing.body?.shards ?? [];

    const expectedHyde = bench.completed * 12;
    checks.codeCompleteHigh = bench.completed >= chunks.length * 0.99;
    checks.hydeCompleteHigh = bench.hyde_completed >= expectedHyde * 0.95;
    checks.speedup5x = bench.chunks_per_sec >= BASELINE_CPS * 5;

    console.log(`completed: code=${bench.completed}/${chunks.length} hyde=${bench.hyde_completed}/${expectedHyde}`);
    console.log(`cps=${bench.chunks_per_sec} vps=${bench.vectors_per_sec} speedup=${bench.speedup_vs_baseline}x`);
    console.log(`deepseek_calls=${bench.deepseek_calls_total} vertex_calls=${bench.vertex_calls_total} failed=${bench.failed}`);
    console.log("per-shard:");
    for (const s of bench.shards) {
      console.log(`  shard ${s.shard_index} sa=${s.sa_index}: code=${s.chunks_done} hyde=${s.hyde_done} ds=${s.deepseek_calls} v=${s.vertex_calls} ds_ms=${s.deepseek_ms} v_ms=${s.vertex_ms} vec=${s.vectorize_ms} d1=${s.d1_ms} err=${s.errors}`);
    }

    bench.finished_at = new Date().toISOString();
    fs.writeFileSync(path.join(pocDir, "bench-30a.json"), JSON.stringify(bench, null, 2), "utf8");
  ...
  console.log(`\nbench-30a.json: ${JSON.stringify(bench, null, 2)}`);
  const allPass = Object.values(checks).every(Boolean);
  console.log(`\n${allPass ? "PASS POC 30A" : "FAIL POC 30A"}`);

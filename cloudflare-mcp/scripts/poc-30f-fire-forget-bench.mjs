#!/usr/bin/env node
/**
 * POC 30F: fire-and-forget producer.
 *
 * Same dual fan-out shape as 30C, but the producer returns immediately
 * (ctx.waitUntil drives the work). Client measures:
 *   - response_ms              (HTTP response time, target <500ms)
 ...
 *
 * Pass criteria:
 *   - response_ms <= 1500ms        (sync R2 put + D1 write should be cheap)
 *   - all 632 code chunks land
 *   - hyde rows >= 95% of expected
 *   - time_to_code_live << time_to_hyde_live (proves decoupling shows up
 *     to the client, not just internally)
 *
 * Run: node cloudflare-mcp/scripts/poc-30f-fire-forget-bench.mjs
 */
...
const pocDir = path.resolve(__dirname, "../poc/30f-fire-forget");
...
const TAG = "30f";
const workerName = `cfcode-poc-${TAG}-fireforget`;
const dbName = `cfcode-poc-${TAG}-fireforget`;
...
async function main() {
  console.log("POC 30F: fire-and-forget producer\n");
  const checks = {
  ...
  const bench = {
    poc: "30f",
    note: "fire-and-forget producer via ctx.waitUntil; client polls /jobs/:id/status",
  ...
  try {
    ...
    console.log("\n--- POST /ingest-sharded (returns immediately) ---");
    const t0 = Date.now();
    ...
    console.log(`response_ms=${responseMs}  status=${ing.status}  body.status=${ing.body?.status}  server_response_ms=${ing.body?.response_ms}`);
    bench.response_ms = responseMs;
    bench.server_response_ms = ing.body?.response_ms;
    checks.responseFast = responseMs <= 1500 && ing.status === 200 && ing.body?.status === "running";
    ...
    if (poll.lastJob) {
      ...
      checks.codeComplete = (poll.lastJob.completed ?? 0) === chunks.length;
      checks.hydeComplete = (poll.lastJob.hyde_completed ?? 0) >= expectedHyde * 0.95;
      // Decoupling visible: code goes live noticeably earlier than hyde (>20s gap on lumae)
      checks.decouplingObservable = poll.timeToCodeLive != null && poll.timeToHydeLive != null
        && poll.timeToHydeLive - poll.timeToCodeLive > 20_000;
    }

    bench.finished_at = new Date().toISOString();
    fs.writeFileSync(path.join(pocDir, "bench-30f.json"), JSON.stringify(bench, null, 2), "utf8");
    checks.benchWritten = true;
    console.log(`\nbench written: ${path.join(pocDir, "bench-30f.json")}`);
    ...
    console.log(`  decouple gap      : ${poll.timeToHydeLive != null && poll.timeToCodeLive != null ? poll.timeToHydeLive - poll.timeToCodeLive : "n/a"}ms`);
  } finally {
    cleanup(configPath);
  ...
  console.log(`\n${allPass ? "PASS POC 30F" : "FAIL POC 30F"}`);

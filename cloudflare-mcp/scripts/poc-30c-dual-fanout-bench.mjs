 ...
 *
 * Producer fires CODE_SHARD_DO and HYDE_SHARD_DO populations as two
 * independent Promise.allSettled fan-outs. Single-purpose DOs.
 *
 * Pass criteria:
 *   - All 632 code chunks complete
 *   - HyDE rows ≥95% of expected
 *   - Code wall_ms (path-only) ≤ HyDE wall_ms (proves dual-fanout independence)
 *   - Total e2e wall_ms ≤ HyDE wall_ms + ~5s overhead
 *
...
const TAG = "30c";
const workerName = `cfcode-poc-${TAG}-dualfan`;
const dbName = `cfcode-poc-${TAG}-dualfan`;
...
async function main() {
  console.log("POC 30C: dual fan-out (code + hyde at producer)\n");
  const checks = {
    resourcesReady: false, deployed: false, secretsSet: false, chunksBuilt: false,
    ingestOK: false, codeComplete: false, hydeComplete: false,
    codePathFastEnough: false, totalE2EReasonable: false,
    benchWritten: false, cleanedUp: false,
  };
  const bench = {
    poc: "30c",
    note: "dual fan-out: code shards + hyde shards as two independent Promise.allSettled populations",
  ...
  try {
    ...
    console.log("\n--- POST /ingest-sharded (dual fan-out, hyde=true) ---");
    const t0 = Date.now();
    const ing = await fetchJson(`${baseUrl}/ingest-sharded`, {
      method: "POST", headers: { "content-type": "application/json" },
      body: JSON.stringify({
        job_id: jobId, repo_slug: repoSlug, indexed_path: lumaePath, active_commit: "30c",
    ...
    const e2e_wall_ms = Date.now() - t0;
    console.log(`status=${ing.status} client_e2e_wall=${e2e_wall_ms}ms`);
    if (ing.status !== 200) console.log(`body: ${JSON.stringify(ing.body).slice(0, 600)}`);
    checks.ingestOK = ing.status === 200 && ing.body?.ok === true;

    bench.code = ing.body?.code ?? null;
    bench.hyde = ing.body?.hyde ?? null;
    bench.total_wall_ms = ing.body?.total_wall_ms ?? e2e_wall_ms;
    bench.client_e2e_wall_ms = e2e_wall_ms;
    bench.status = ing.body?.status;

    if (bench.code) {
      console.log(`code: completed=${bench.code.completed}/${chunks.length} wall=${bench.code.wall_ms}ms cps=${bench.code.chunks_per_sec} errors=${bench.code.errors}`);
      checks.codeComplete = bench.code.completed === chunks.length && bench.code.errors === 0;
      // code path should finish in <= ~15s on lumae (matches 29F=8.2s + variance)
      checks.codePathFastEnough = bench.code.wall_ms <= 15000;
    }
    if (bench.hyde) {
      const expected = chunks.length * 12;
      console.log(`hyde: completed=${bench.hyde.hyde_completed}/${expected} wall=${bench.hyde.wall_ms}ms vps=${bench.hyde.vectors_per_sec} errors=${bench.hyde.errors}`);
      checks.hydeComplete = bench.hyde.hyde_completed >= expected * 0.95;
    }
    // Total e2e should be ~max(code_wall, hyde_wall) plus small overhead
    checks.totalE2EReasonable = bench.total_wall_ms <= Math.max(bench.code?.wall_ms ?? 0, bench.hyde?.wall_ms ?? 0) + 5000;

    bench.finished_at = new Date().toISOString();
    fs.writeFileSync(path.join(pocDir, "bench-30c.json"), JSON.stringify(bench, null, 2), "utf8");
    checks.benchWritten = true;
    console.log(`\nbench written: ${path.join(pocDir, "bench-30c.json")}`);
  } finally {
    cleanup(configPath);
  ...
  console.log(`\nbench-30c.json: ${JSON.stringify(bench, null, 2).slice(0, 1200)}...`);
  const allPass = Object.values(checks).every(Boolean);
  console.log(`\n${allPass ? "PASS POC 30C" : "FAIL POC 30C"}`);

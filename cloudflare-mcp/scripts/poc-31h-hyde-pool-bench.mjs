#!/usr/bin/env node
// POC 31H: HyDE with decoupled embedding pool + all 4 SAs
...
const __dir = path.dirname(fileURLToPath(import.meta.url)), root = path.resolve(__dir, "../.."), poc = path.resolve(__dir, "../poc/31h-hyde-pool");
const T = "31h", wn = `cfcode-poc-${T}-hpool`, dn = `cfcode-poc-${T}-hpool`, r2b = `cfcode-poc-${T}-hpool`, vi = `cfcode-poc-${T}-hpool`;
const sas = ["team (1).json", "underwriter-agent-479920-af2b45745dac.json", "big-maxim-331514-b90fae4428bc.json", "embedding-code-495015-2fa24eece6fa.json"].map(f => "/Users/awilliamspcsevents/.config/cfcode/sas/" + f);
...
async function main() {
  ...
  for (let i = 0; i < 4; i++) { const sn = `GEMINI_SERVICE_ACCOUNT_B64${i === 0 ? "" : "_" + (i + 1)}`, sv = Buffer.from(fs.readFileSync(sas[i], "utf8")).toString("base64"); if (spawnSync("npx", ["wrangler", "secret", "put", sn, "--config", cp], { cwd: poc, env, input: sv, encoding: "utf8" }).status !== 0) throw new Error("secret " + sn + " failed"); }
  ...
  console.log("\n--- POST /ingest-sharded (4 SAs, embed-pool) ---"); const t0 = Date.now();
  const ing = await (await fetch(u + "/ingest-sharded", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ job_id: jid, repo_slug: repoS, indexed_path: luma, active_commit: T, artifact_key: ak, artifact_text: at, code_shard_count: 4, hyde_shard_count: 16, code_batch_size: 500, hyde_batch_size: 500, embed_batch_size: 500, num_sas: 4, hyde: true }) })).json();
  ...
  console.log((pass ? "PASS" : "FAIL") + " POC 31H (hyde=" + (job?.hyde_completed || 0) + "/" + chunks.length * 12 + ")");
  try { run("npx", ["wrangler", "delete", "--name", wn, "--force"], { q: 1, ok: 1 }); run("npx", ["wrangler", "vectorize", "delete", vi, "--force"], { q: 1, ok: 1 }); run("npx", ["wrangler", "r2", "bucket", "delete", r2b], { q: 1, ok: 1 }); run("npx", ["wrangler", "d1", "delete", dn, "--skip-confirmation"], { q: 1, ok: 1 }); } catch {}

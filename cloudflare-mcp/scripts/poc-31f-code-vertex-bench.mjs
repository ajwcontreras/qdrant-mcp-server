#!/usr/bin/env node
// POC 31F: Code-only Vertex + Vectorize on alarm fan-out. Real lumae chunks.
// Smoke: node cloudflare-mcp/scripts/poc-31f-code-vertex-bench.mjs
...
const __dir = path.dirname(fileURLToPath(import.meta.url)), r2 = path.resolve(__dir, "../.."), poc = path.resolve(__dir, "../poc/31f-code-vertex");
const T = "31f", wn = `cfcode-poc-${T}-codev`, dn = `cfcode-poc-${T}-codev`, r2b = `cfcode-poc-${T}-codev`, vi = `cfcode-poc-${T}-codev`;
...
for (const l of fs.readFileSync(r2 + "/.cfapikeys", "utf8").split(/\r?\n/)) { const [k, ...r] = l.split("="); const v = r.join("=").trim(); if (k.trim() === "CF_GLOBAL_API_KEY") env.CLOUDFLARE_API_KEY = v; if (k.trim() === "CF_EMAIL") env.CLOUDFLARE_EMAIL = v; }
function run(cmd, args, o = {}) { const R = spawnSync(cmd, args, { cwd: o.c || poc, env, encoding: "utf8", stdio: o.q ? ["ignore", "pipe", "pipe"] : "inherit", input: o.i }); if (R.status !== 0 && !o.ok) throw new Error(`${cmd} ${args.join(" ")} failed\n${R.stdout}\n${R.stderr}`); return R; }
...
async function main() {
  ...
  const sa1b = Buffer.from(fs.readFileSync(sa1, "utf8")).toString("base64");
  const sa2b = Buffer.from(fs.readFileSync(sa2, "utf8")).toString("base64");
  for (const [k, v] of [["GEMINI_SERVICE_ACCOUNT_B64", sa1b], ["GEMINI_SERVICE_ACCOUNT_B64_2", sa2b]]) { if (spawnSync("npx", ["wrangler", "secret", "put", k, "--config", cp], { cwd: poc, env, input: v, encoding: "utf8" }).status !== 0) throw new Error("secret " + k + " failed"); }
  ...
  const ing = await (await fetch(u + "/ingest-sharded", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ job_id: jid, repo_slug: repoS, indexed_path: luma, active_commit: T, artifact_key: ak, artifact_text: at, code_shard_count: 4, code_batch_size: 500, hyde: false }) })).json();
  console.log("response_ms=" + (Date.now() - t0) + " status=" + ing.status + " chunks=" + (ing.chunks || "?"));

  const dl = Date.now() + 60000; let job = null;
  while (Date.now() < dl) { const r = await (await fetch(u + "/jobs/" + jid + "/status")).json(); if (r.ok && r.job) { job = r.job; process.stdout.write("\r  " + job.code_status + "/" + job.completed + "/" + job.total + "  st=" + job.status + "  "); if (job.status !== "running" && job.code_status !== "running") break; } await new Promise(r => setTimeout(r, 1000)); }
  console.log("");
  const pass = job && job.status === "published" && job.completed === chunks.length;
  console.log((pass ? "PASS" : "FAIL") + " POC 31F");
  const keepOnFail = process.argv.includes("--keep");
  try { if (!keepOnFail || pass) { run("npx", ["wrangler", "delete", "--name", wn, "--force"], { q: 1, ok: 1 }); run("npx", ["wrangler", "vectorize", "delete", vi, "--force"], { q: 1, ok: 1 }); run("npx", ["wrangler", "r2", "bucket", "delete", r2b], { q: 1, ok: 1 }); run("npx", ["wrangler", "d1", "delete", dn, "--skip-confirmation"], { q: 1, ok: 1 }); } else { console.log("KEEPING resources for debug (--keep): " + u); } } catch {}
